...
- 记录检索能力不足：缺少多条件组合筛选与用户可选排序。
- 记录维护能力有限：缺少交易记录编辑能力（当前仅支持删除）。
- 缺少预算管理与超支提醒：暂不支持按月预算、阈值预警与超支告警。
- 缺少显式备份与恢复能力：暂不支持手动导出备份与导入恢复。
- 导入质量与异常提示待标准化：三类导入链路的错误结构和提示粒度需统一。

详细需求请参考需求规格说明书中的 FR-02、FR-03、FR-09、FR-10、FR-11。
